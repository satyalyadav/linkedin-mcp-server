...
# explicit per-locale table and fails closed on unknown locales.
#
# The control exists only on an open posting the account has not applied to.
# A closed posting renders none, and its saved job is exactly the kind a
# caller wants to unsave, so this path cannot serve those; the saved-jobs
# tracker (`/jobs-tracker/?stage=saved`) carries an Unsave menu item instead.
