...
class JobScraper:
    ...
    async def unsave_job(self, job_id: str) -> dict[str, Any]:
        ...
        Idempotent, and limited to the posting's own Save control: a closed
        posting renders none, so a saved job that closed cannot be unsaved
        through this route.
        """
        job_id = normalize_job_id(job_id)
        url = job_view_url(job_id, "/")
        ...
        clicked = await self._pages.click_save_control("saved")
        if not clicked:
            raise LinkedInScraperException(
                "Could not find or click the job's Save control in its saved state."
            )
        ...
        await asyncio.sleep(1)
        if await self._pages.save_control_state() != "unsaved":
            raise LinkedInScraperException(
